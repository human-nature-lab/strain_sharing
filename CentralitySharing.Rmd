...
lm.1 <- lmerTest::lmer(sim_deg_one ~ cluster_coef_all + age_at_survey +household_wealth_index_w3 +
                         gender +  DNA_conc + BMI + sampling_date + shipment_batch + Plate + WIKI + Reads +(1|village_name),
             data = centrality_sim_df_covars_all)
summary(lm.1)
```
```{r}
lm.1 <- lmerTest::lmer(sim_deg_one ~ eigen_c_all + age_at_survey +household_wealth_index_w3 +
                         gender+ DNA_conc + BMI + sampling_date + shipment_batch + Plate + WIKI + Reads +(1|village),
             data = centrality_sim_df_covars_all)
summary(lm.1)
```
```{r}
...
lm.1 <- lmerTest::lmer(sim_deg_one ~ degree_c_all + age_at_survey +household_wealth_index_w3 +
                         gender+  DNA_conc + BMI + l0400 + mb_m0300 + sampling_date + shipment_batch + Plate + WIKI + Reads +(1|village),
             data = centrality_sim_df_covars_all)
summary(lm.1)
```

```{r}
lm.1 <- lmerTest::lmer(sim_deg_one ~ degree_c_all + BMI + age_at_survey + household_wealth_index_w3 + gender+  DDS +med_painkillers +med_antibiotics+ med_antidiarrheal+ med_antiparasite+ med_vitamins+ med_zinc+ med_antifungal+ med_antihypertensives+ med_antidiabetics+ med_antiacids+ med_laxatives+ l0400+ mb_m0300 +
                          DNA_conc + sampling_date + shipment_batch + Plate + WIKI + Reads +(1|village) + (1|building_id),
...
```{r}
lm.1 <- lmerTest::lmer(sim_deg_one ~ degree_c_all + maha_social +
                          DNA_conc + sampling_date + shipment_batch + Plate + WIKI + Reads +(1|village) + (1|building_id),
             data = bind_cols(centrality_sim_df_covars_all, maha_social = mahalanobis(sub_covar_centrality, FALSE, var(sub_covar_centrality, na.rm=TRUE)))
       )
summary(lm.1)
