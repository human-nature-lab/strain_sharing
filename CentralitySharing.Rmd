...
```{r}
count <- 0
#Where are we losing the people from?
length(unique(c(SN$ego, SN$alter)))
#From the few villages we dropped?
for(i in 1:length(village_names)){
  sn_vil <- SN %>% filter(village_name_w3 == village_names[i])
  sn_vil_all <- all_edges %>% filter(village_name_w3 == village_names[i])
...
#centrality_sim_df <- centrality_df
...
centrality_sim_df_covars_all <- merge(centrality_sim_df_covars, shiv_covar_df)

#We lose 240 people for not having covariates, look into this
head(centrality_sim_df_covars_all)
...
deg_c_sharing_fig <- ggarrange(deg_c_sharing_all,deg_c_sharing_first,
                               labels = c("4a", "4b"))
...
lm.1 <- lmerTest::lmer(strain_sim_all ~ degree_c + age_at_survey +household_wealth_index_w3  + birth_in_village+
                         gender+ (1|village),
             data = centrality_sim_df_covars)
...
lm.1 <- lmerTest::lmer(sim_deg_one ~ cluster_coef_all + age_at_survey +household_wealth_index_w3 +
                         gender+ dna_conc + BMI +(1|village_name),
             data = centrality_sim_df_covars_all)
summary(lm.1)

lm.1 <- lmerTest::lmer(sim_deg_one ~ eigen_c_all + age_at_survey +household_wealth_index_w3 +
                         gender+ dna_conc + BMI +(1|village),
             data = centrality_sim_df_covars_all)
summary(lm.1)
...
lm.1 <- lmerTest::lmer(invsimpson ~ between_c_all + age_at_survey +household_wealth_index_w3 +
             ...
             data = centrality_sim_df_covars_all)
summary(lm.1)
names(centrality_sim_df_covars_all)
...
immigrant_df <- centrality_sim_df_covars %>% filter(!(b0800 %in% c("Dont_Know", "Less than a year")))


lm.1 <- lmerTest::lmer(strain_sim_all ~ degree_c + age_at_survey +household_wealth_index_w3 +
                         gender+  b0800+ (1|village),
             data = immigrant_df)
...
man_woman_df.m = reshape2::melt(man_woman_df %>% select(-Jaccard_Similarity), id.var=c("Gender"))

man_woman_df.m = reshape2::melt(man_woman_df, id.var=c("Gender"))


levels(man_woman_df.m$variable) <- c("Strain-Sharing Rate", "Bray-Curtis Similarity", "Jaccard Similarity")
...
mw_share_pvals <- mw_share_pvals[c(1,10,15),] 

mw_share_pvals$p.adj <- mw_share_pvals$p %>% p.adjust(method = "BH")

mw_share_pvals$p.adj
...
gender_split_share <- ggboxplot(SN_tri_gender, color = "pair_gender",
  ...
  stat_compare_means(label.y = c(.20,.25,.30),
...
ggboxplot(SN_tri_gender, color = "pair_gender",
  ...
  theme(#axis.text.x = element_text(angle = 45, hjust=1),
    ...
    legend.position= "bottom",
    legend.margin=margin(0,0,0,0, unit='cm')) +
  stat_compare_means(label.y = c(.6,.65,.7),
                     comparisons = my_comparisons,
...
#compare_means(jaccard_sim ~  relationship + pair_gender,
#                     SN_tri_gender,
 #             p.adjust.method = "BH")
...
wilcox.test(SN_tri_gender$bray_curtis_sim[SN_tri_gender$pair_gender == "Male - Male" &
                                            SN_tri_gender$relationship == "Sibling"],
            SN_tri_gender$bray_curtis_sim[SN_tri_gender$pair_gender == "Female - Female" &
                                            SN_tri_gender$relationship == "Sibling"])$p.value * 12*3

# tri_gender_df.m <- SN_tri_gender %>% select(bray_curtis_sim, jaccard_sim, strain_sharing_rate, pair_gender)
# tri_gender_df.m <- reshape2::melt(tri_gender_df.m, id.var=c("pair_gender"))
# head(tri_gender_df.m)
# tri_gender_df.m$all_group <- paste0(tri_gender_df.m$pair_gender,tri_gender_df.m$variable)
# 
# tri_gender_df_pvals <- compare_means(value ~ all_group, data = tri_gender_df.m)
# 
# tri_gender_df_pvals $>$ filter(group1 == )
```
...
gender_fig <- ggarrange(gender_split_share +
                   #rremove("xlab") +
                   rremove("ylab"),
          ggarrange(mw_share +
                      rremove("ylab") ,
