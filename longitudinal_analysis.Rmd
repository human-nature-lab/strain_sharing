...
strain_shared |> 
  ...
  group_by(pair, timepoint, village_code_ego) |>  
  summarize(ssr=sum(trans==TRUE)/n())
  ggplot() + 
    geom_boxplot(aes(as.factor(timepoint), ssr))
...
strain_shared |> 
  ...
  mutate(trans = dist <= threshold) |> 
  mutate(timepoint = 'Intra-timepoint') |>
  group_by(species, village_code_ego) |>
  summarize(sum(trans) / n(), n = n()) |>
  filter(n>10) |>
  arrange(desc(n)) |>View()
```
...
foreach(i = 1:4) %do% {
  ...
  sharing_rate |> 
    ...
    wilcox.test(delta_ssr ~ tie, ratio)
