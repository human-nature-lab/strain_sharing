...
```{r eval=FALSE, include=FALSE}
lmer_species_rand_expanded <- glmer(T2_t ~ T1_t + relationship + relationship + gender.mm + gender.ff + indigenous.both + indigenous.one + age_difference_abs + average_age + average_age + DDS_difference_abs + (1|species) + (1|village_code_ego) + (1|ego), family = binomial, nAGQ=0,data = xcov, )
```
...
lmer_first_model_expanded  <- lmer(T2 ~ T1 + relationship + gender.mm + gender.ff + indigenous.both + indigenous.one + age_difference_abs + average_age + average_age + DDS_difference_abs  + (1|village_code_ego) + (1|ego), data = sharing_rate_followup_metadata)
