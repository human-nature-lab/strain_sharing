...
xcov <- xcov[xcov$village_code_w3_alter==xcov$village_code_w3_ego, ]
```
...
doParallel::registerDoParallel(cl)
foreach(s = unique(xcov$species)) %dopar% {
  tmpdata = xcov[xcov$species==s,]
  villages_to_keep = tmpdata |> group_by(village_code_w3_ego, species) |> count() |> filter(n>1) |> pull(village_code_w3_ego)
  tmpdata = tmpdata[tmpdata$village_code_w3_ego %in% villages_to_keep,]
lme4::glmer(data = tmpdata, 
                 T2_t ~ T1_t*intra_ego_t*intra_alter_t + gender.mm + gender.ff + 
                   ...
                   relationship + (0+T1_t|village_code_w3_ego), 
                 family = binomial
                 )
} -> long_glmm
parallel::stopCluster(cl)
