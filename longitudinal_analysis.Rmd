...
metadata_followup <- read_csv('~/Documents/F_BEGHINI_HM/COHORT1_FOLLOWUP/v2/hmb_respondents_cohort1_followup_v2_f_beghini_2022-11-08.csv', na = c('NA','Dont_Know','Refused'),show_col_types = FALSE) |> 
  mutate(mb_m05 = str_replace(mb_m00, '-00$', '-15'),
  marital_name = if_else(!is.na(mb_c004),mb_c004, 'Married')) |> 
  filter(!is.na(mb_m05))
metadata_baseline <- read_csv('~/Documents/F_BEGHINI_HM/COHORT1/v3/hmb_respondents_cohort1_baseline_v3_f_beghini_2022-11-08.csv', na = c('NA','Dont_Know','Refused'),show_col_types = FALSE) |> 
...
SN_w1 <- read_csv("~/Documents/F_BEGHINI_G/WAVE1/v8/honduras_connections_WAVE1_v8_F_BEGHINI_2023-11-01.csv",show_col_types = FALSE) |> 
...
SN_w3 <- read_csv("~/Documents/F_BEGHINI_G/WAVE3/v3/honduras_connections_WAVE3_v3_F_BEGHINI_2021-09-27.csv", guess_max = 50000,show_col_types = FALSE) |> 
              filter(relationship %in% c('child_over12_other_house','closest_friend','father','free_time','mother','partner','personal_private','sibling'), #Remove not get along
                     ego %in% metadata_followup$respondent_master_id, 
                     alter %in% metadata_followup$respondent_master_id,
                     ego %in% metadata_baseline$respondent_master_id, 
...
SN_w4 <- read_csv("~/Documents/F_BEGHINI_G/WAVE4/v1/honduras_connections_WAVE4_v1_F_BEGHINI_2023-11-01.csv", guess_max = 50000,show_col_types = FALSE) |> 
              filter(relationship %in% c('child_over12_other_house','closest_friend','father','free_time','mother','partner','personal_private','sibling'), #Remove not get along
                     ego %in% metadata_followup$respondent_master_id, 
                     alter %in% metadata_followup$respondent_master_id) |> 
                mutate(pair = vapply(stri_split_boundaries(str_c(ego, alter), type='character'), function(x) stri_c(x[stri_order(x)], collapse = ''), ''),
                       wave = 4)
respondents_w4 <- read_csv('~/Documents/F_BEGHINI_G/WAVE4/v1/honduras_respondents_WAVE4_v1_F_BEGHINI_2023-11-01.csv', guess_max = 50000,show_col_types = FALSE) |> 
              filter(respondent_master_id %in% metadata_followup$respondent_master_id)
```
