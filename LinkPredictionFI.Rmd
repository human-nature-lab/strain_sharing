...
for(i in 1:5){
  ...
  logit.test.1 <- lme4::glmer(related ~ strain_sharing_rate + (0+strain_sharing_rate|village_name_w3),
                              data = train1, family = binomial)
  logit.test.2 <- lme4::glmer(related ~ strain_sharing_rate + (0+strain_sharing_rate|village_name_w3),
                              data = train2, family = binomial)
  logit.test.3 <- lme4::glmer(related ~ strain_sharing_rate + (0+strain_sharing_rate|village_name_w3),
  ...
  logit.test.1.covars <- lme4::glmer(related ~ strain_sharing_rate + gender.mm + gender.mf +
                        age_difference_abs + average_age + (0+strain_sharing_rate|village_name_w3),
                              data = train1, family = binomial)
  ...
  logit.test.1.all <- lme4::glmer(related ~ gender.mm + gender.mf + indigenous.both +
                        ...
                        strain_sharing_rate + (0+strain_sharing_rate|village_name_w3),
                              data = train1, family = binomial)
...
Prediction plot all rels
```{r}
village_map
#Ah that's it
#See which villages have the best accuracy
Downsamp_all %>% group_by(village_name_w3) %>% summarize(acc = mean(predicted == related),
...
Downsamp_all %>% filter(related == 1)%>% group_by(village_name_w3) %>%
  summarize(acc = mean(predicted == related))

Downsamp_all_nkh %>% group_by(village_name_w3) %>% summarize(acc = mean(predicted == related))
...
pred_graph <- Downsamp_all %>%
  filter(village_name_w3 == "Carrizalito 1", related ==1) %>%
...
#Fuck shouldn't this be simplified
...
svglite("../FiguresNew/Figure3/fig3_full_updated.svg")
fig3
dev.off()
...
#  select(-c(ego, alter, tp, bray_curtis_sim, jaccard_sim, village_name_w3, predicted))

Downsamp_all_importance <- Downsamp_all %>%
  select(-c(ego, alter, bray_curtis_sim, jaccard_sim, village_name_w3, predicted))
...
colnames(auc_perms_ssr) <- c("gender", "indigenous_status", "religion", "age_difference","average_age","wealth_difference", "average_wealth",
                             "education_difference","average_education","strain_sharing_rate")
...
svglite("../FiguresNew/SFigure9/sfigure9_full.svg")
...
Downsamp_all_importance <- Downsamp_all %>%
  select(-c(ego, alter, tp, strain_sharing_rate, bray_curtis_sim, village_name_w3, predicted))
