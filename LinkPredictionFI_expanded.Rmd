...
foreach(v = village_names, .combine = rbind) %dopar% {
  ...
  print(v)
  Village_NoRels
} -> All_NoRel_Covars     #Get covariates for people without relationship ties

parallel::stopCluster(cl)

#Set Un-nominated same village relationship
...
```{r}
#Illustrative visualizations
Downsamp_all <- Downsamp_all %>% mutate(tp = predicted == related)


pred_graph <- Downsamp_all %>%
  filter(village_code_w3 == 145 , related ==1) %>%
  graph_from_data_frame(directed = FALSE) 

pred_colors <- edge_attr(pred_graph, "predicted")
E(pred_graph)$size <- pred_colors*1.5+3.5

pred_colors[pred_colors == 1] <- "#027fffff"
...
Prediction plot nkh
```{r}
Downsamp_all_nkh <- Downsamp_all_nkh %>% mutate(tp = predicted == related)

pred_graph_nkh <- Downsamp_all_nkh %>%
  filter(village_code_w3 == 145, related ==1) %>%
  graph_from_data_frame(directed = FALSE)
...
fig3 <- ggarrange(networks_1, rocs_1, rocs_1, ncol = 3)

svglite("Figures/fig3_a.svg")
p1_test
dev.off()

svglite("Figures/fig3_b.svg")
p2_test
dev.off()

svglite("Figures/fig3_full_updated_extended.svg", fix_text_size = FALSE, width = 14, height = 8)
fig3
dev.off()
...
```{r}
r2_coefs  <- bind_rows(as.data.frame(r2glmm::r2beta(llm_non_kin_for2, method = 'nsj')) |> bind_cols('Relationship' = 'Non kin'),
as.data.frame(r2glmm::r2beta(llm_all_kin_for2, method = 'nsj')) |> bind_cols('Relationship' = 'All relationships')) 

Effect_Rsq_plot  <- ggplot(r2_coefs, aes(reorder(Effect, Rsq), Rsq, fill = Relationship)) + 
...
geom_errorbar(
  ...
  xlab('Covariate') + ylab(bquote(R^2)) +
  coord_flip() +
...
for(i in 1:100){
  ...
  logit_perm <- glm(related ~ . ,family = binomial, data = dat_permuted)
  auc_perms_ssr$different_diet[i] <- auc(dat_permuted$related , logit_perm$fitted.values,                                                levels=c("0","1"), direction = "<")
...
for(i in 1:100){
  ...
  logit_perm <- glm(related ~ . ,family = binomial, data = dat_permuted)
  auc_perms_ssr$average_diet[i] <- auc(dat_permuted$related , logit_perm$fitted.values,                                                levels=c("0","1"), direction = "<")
...
for(i in 1:100){
  ...
  logit_perm <- glm(related ~ . ,family = binomial, data = dat_permuted)
  auc_perms_ssr$different_bristol[i] <- auc(dat_permuted$related , logit_perm$fitted.values,                                                levels=c("0","1"), direction = "<")
...
for(i in 1:100){
  ...
  logit_perm <- glm(related ~ . ,family = binomial, data = dat_permuted)
  auc_perms_ssr$average_bristol[i] <- auc(dat_permuted$related , logit_perm$fitted.values,                                                levels=c("0","1"), direction = "<")
...
for(i in 1:100){
  ...
  logit_perm <- glm(related ~ . ,family = binomial, data = dat_permuted)
  auc_perms_ssr$watersource_same[i] <- auc(dat_permuted$related , logit_perm$fitted.values,                                                levels=c("0","1"), direction = "<")
...
for(i in 1:100){
  ...
  logit_perm <- glm(related ~ . ,family = binomial, data = dat_permuted)
  auc_perms_ssr$same_med_usage[i] <- auc(dat_permuted$related , logit_perm$fitted.values,                                                levels=c("0","1"), direction = "<")
...
svglite("Figures/sfigure9_full.svg", width = 16, height = 12, bg = 'transparent',fix_text_size = FALSE)
...
# Flip relationship and strain sharing rate

# Permutation feature importance: all relationships strain sharing prediction
```{r}
Downsamp_all_importance <- Downsamp_all %>%
  select(-c(ego, alter, bray_curtis_sim, predicted))

readr::write_tsv(Downsamp_all_importance, 'data/export/Downsamp_all_importance.tsv')
...
readr::write_tsv(Downsamp_all_importance, 'data/export/Downsamp_all_importance_nk.tsv')

mse_perms_ssr <- data.frame(matrix(nrow = 1000, ncol = 17))
colnames(mse_perms_ssr) <- c("gender", "indigenous_status", "religion", "age_difference","average_age","wealth_difference", "average_wealth",
...
for(i in 1:1000){
  ...
  mse_perms_ssr$household_same[i] <- 0
...
fi_all <- ggplot(mse_perms_ssr_dat_all_rel) +
  geom_bar(
    aes(x = forcats::fct_relevel(name, levels(reorder(mse_perms_ssr_dat_nk$name, -mse_perms_ssr_dat_nk$value))), y = value),
    # aes(x = forcats::fct_reorder2(name, mse_perms_ssr_dat_nk$name, mse_perms_ssr_dat_nk$value), y = value),
    stat = "identity",
    fill = "skyblue",
    alpha = 0.7
  ) +
...
svglite('Figures/sfigure22.svg', fix_text_size = FALSE)
Effect_Rsq_plot
dev.off()
