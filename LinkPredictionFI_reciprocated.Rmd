---
title: "Link Prediction only on reciprocated ties"
...
SN_All_Covars_reciprocated_ties <- SN |>
...
village_names <- unique(SN_All_Covars_reciprocated_ties$village_code_w3)
foreach(v = village_names, .combine = rbind) %do%
{
  #Split up matrix into villages
  SN_Village_w3 <- SN_All_Covars_reciprocated_ties %>% filter(village_code_w3 == v)
  ...
  data.frame(t(combn(Village_IDs_w3, 2, simplify = TRUE))) |> 
    ...
    ungroup()
} -> All_NoRel_Covars_reciprocated


#Get dataframe of relationships into format for regression
SN_All_Covars_for_Regression_reciprocated_ties <- SN_All_Covars_reciprocated_ties %>% distinct(pair_key, .keep_all = TRUE) %>%
...
```{r message=FALSE, warning=FALSE, include=FALSE}
family_house_pairs_reciprocated <- SN_All_Covars_reciprocated_ties |>
...
SN_Non_Kin_Dif_House_Covars_for_Regression_reciprocated_ties<- SN_All_Covars_for_Regression_reciprocated_ties %>%
  filter(!(pair_key %in% family_house_pairs_reciprocated))


#select predictors
SN_All_Covars_for_Regression_reciprocated_ties <- SN_All_Covars_for_Regression_reciprocated_ties %>%
  mutate(related = 1) %>% 
...
SN_Non_Kin_Dif_House_Covars_for_Regression_reciprocated_ties <- SN_Non_Kin_Dif_House_Covars_for_Regression_reciprocated_ties %>%
  mutate(related = 1) %>% 
  dplyr::select("ego","alter","gender.mm", "gender.mf", "indigenous.both", "indigenous.one", "religion.same",
         "age_difference_abs", "average_age", "wealth_difference_abs",
         "average_wealth", "education_difference_abs","average_education", "household_same",
...
All_NoRel_Covars_for_Regression_reciprocated_ties<- All_NoRel_Covars_reciprocated %>%
  mutate(related = 0) %>%
  dplyr::select("ego","alter","gender.mm", "gender.mf", "indigenous.both", "indigenous.one", "religion.same",
                "age_difference_abs", "average_age", "wealth_difference_abs",
                "average_wealth", "education_difference_abs","average_education", "household_same",
...
SN_All_Covars_for_Regression_complete_reciprocated_ties <- SN_All_Covars_for_Regression_reciprocated_ties[complete.cases(SN_All_Covars_for_Regression_reciprocated_ties),]

SN_Non_Kin_Dif_House_Covars_for_Regression_complete_reciprocated_ties <- SN_Non_Kin_Dif_House_Covars_for_Regression_reciprocated_ties[complete.cases(SN_Non_Kin_Dif_House_Covars_for_Regression_reciprocated_ties),]

All_for_Regression_reciprocated_ties <- rbind(All_NoRel_Covars_for_Regression_reciprocated_ties, SN_All_Covars_for_Regression_reciprocated_ties)

Non_Kin_House_for_Regression_reciprocated_ties <- rbind(All_NoRel_Covars_for_Regression_reciprocated_ties,
                                      SN_Non_Kin_Dif_House_Covars_for_Regression_complete_reciprocated_ties)

All_for_Regression_complete_reciprocated_ties <- All_for_Regression_reciprocated_ties[complete.cases(All_for_Regression_reciprocated_ties), ]

All_NoRel_Covars_for_Regression_complete_reciprocated_ties <- All_NoRel_Covars_for_Regression_reciprocated_ties[complete.cases(All_NoRel_Covars_for_Regression_reciprocated_ties), ]

Non_Kin_House_for_Regression_complete_reciprocated_ties <-
  Non_Kin_House_for_Regression_reciprocated_ties[complete.cases(Non_Kin_House_for_Regression_reciprocated_ties), ]

edge_counts <- SN_All_Covars_for_Regression_complete_reciprocated_ties %>%
...
edge_counts_nkh <- SN_Non_Kin_Dif_House_Covars_for_Regression_complete_reciprocated_ties %>%
  group_by(village_code_w3) %>%
  summarize(edge_count = sum(related))
...
foreach(v = names(edge_counts), .combine = rbind) %do% {
  All_NoRel_Covars_for_Regression_complete_reciprocated_ties |>
  filter(village_code_w3 == as.integer(v)) |>
  slice_sample(n = edge_counts[[v]])
} -> Downsamp_all_reciprocated_ties
Downsamp_all_reciprocated_ties <- as.data.frame(Downsamp_all_reciprocated_ties)

Downsamp_all_reciprocated_ties <- rbind(Downsamp_all_reciprocated_ties, SN_All_Covars_for_Regression_complete_reciprocated_ties)

Downsamp_all_reciprocated_ties <- Downsamp_all_reciprocated_ties[complete.cases(Downsamp_all_reciprocated_ties),]
...
} -> Downsamp_all_reciprocated_ties_nkh

Downsamp_all_reciprocated_ties_nkh <- as.data.frame(Downsamp_all_reciprocated_ties_nkh)
Downsamp_all_reciprocated_ties_nkh <- rbind(Downsamp_all_reciprocated_ties_nkh, SN_Non_Kin_Dif_House_Covars_for_Regression_complete_reciprocated_ties)

Downsamp_all_reciprocated_ties_nkh <- Downsamp_all_reciprocated_ties_nkh[complete.cases(Downsamp_all_reciprocated_ties_nkh),]
```
...
foreach(i = c(1:5)) %dopar% {
  print(i)
  inds <- c(1:nrow(Downsamp_all_reciprocated_ties))
  test1_inds <- sample(inds, size = floor(.33*nrow(Downsamp_all_reciprocated_ties)), replace = F)
  ...
  train1 <- Downsamp_all_reciprocated_ties[train1_inds,]
  test1 <- Downsamp_all_reciprocated_ties[test1_inds,]
  train2 <- Downsamp_all_reciprocated_ties[train2_inds,]
  test2 <- Downsamp_all_reciprocated_ties[test2_inds,]
  train3 <- Downsamp_all_reciprocated_ties[train3_inds,]
  test3 <- Downsamp_all_reciprocated_ties[test3_inds,]
...
Downsamp_all_reciprocated_ties$predicted <- pred.all.acc

logit.base.roc <- pROC::roc(Downsamp_all_reciprocated_ties$related, pred.all, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")
logit.onlycovars.roc <- pROC::roc(Downsamp_all_reciprocated_ties$related, pred.all.onlycovars, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")
logit.covars.roc <- pROC::roc(Downsamp_all_reciprocated_ties$related, pred.all.covars, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")
logit.all.roc <- pROC::roc(Downsamp_all_reciprocated_ties$related, pred.all.all, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")
...
dat.ci.list <- lapply(ci.list, function(ciobj) 
  data.frame(x = as.numeric(rownames(ciobj)),
             ...
             upper = ciobj[, 3]))
```
## All Social or Familial Relationship (on reciprocated ties W3)
```{r plot_roc_all_famililar_social_reciprocated_ties, echo=FALSE, message=FALSE, warning=FALSE}
roc.plot.reciprocated <- ggroc(roc.list) +
  theme_minimal() +
  geom_abline(slope=1, intercept = 1, linetype = "dashed", alpha=1, color = "grey") +
  coord_equal()

for(i in 1:4) {
  roc.plot.reciprocated <- roc.plot.reciprocated + geom_ribbon(
    data = dat.ci.list[[i]],
...
roc.plot.reciprocated <- roc.plot.reciprocated + 
theme(plot.title = element_text(size=10, hjust = .5),
  ...
  ggtitle("All Social or Familial Relationships (reciprocated ties)")
...
roc.plot.reciprocated <- roc.plot.reciprocated + theme(
  plot.title = element_text(size = 10, hjust = .5),
  legend.title = element_text(size = 8),
  legend.text = element_text(size = 8)
)

svglite('Figures/plot_roc_all_famililar_social_reciprocated_ties.svg')
roc.plot.reciprocated
dev.off()

png('Figures/plot_roc_all_famililar_social_reciprocated_ties.png', width = 900, height = 900)
roc.plot.reciprocated
dev.off()
```
...
doParallel::registerDoParallel(cl)
foreach(i = c(1:5)) %dopar% {
  inds <- c(1:nrow(Downsamp_all_reciprocated_ties_nkh))
  test1_inds <- sample(inds, size = floor(.33*nrow(Downsamp_all_reciprocated_ties_nkh)), replace = F)
  train1_inds <- inds[!(inds %in% test1_inds)]
  test2_inds <- sample(train1_inds, size = floor(.5*length(train1_inds)), replace = F)
  train2_inds <- inds[!(inds %in% test2_inds)]
  test3_inds <- train2_inds[!(train2_inds %in% test1_inds)]
  ...
  train1 <- Downsamp_all_reciprocated_ties_nkh[train1_inds,]
  test1 <- Downsamp_all_reciprocated_ties_nkh[test1_inds,]
  train2 <- Downsamp_all_reciprocated_ties_nkh[train2_inds,]
  test2 <- Downsamp_all_reciprocated_ties_nkh[test2_inds,]
  train3 <- Downsamp_all_reciprocated_ties_nkh[train3_inds,]
  test3 <- Downsamp_all_reciprocated_ties_nkh[test3_inds,]
...
Downsamp_all_reciprocated_ties_nkh$predicted <- pred.all.acc

logit.base.roc <- pROC::roc(Downsamp_all_reciprocated_ties_nkh$related, pred.all, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")
logit.onlycovars.roc <- pROC::roc(Downsamp_all_reciprocated_ties_nkh$related, pred.all.onlycovars, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")
logit.covars.roc <- pROC::roc(Downsamp_all_reciprocated_ties_nkh$related, pred.all.covars, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")
logit.all.roc <- pROC::roc(Downsamp_all_reciprocated_ties_nkh$related, pred.all.all, ci=TRUE, plot=FALSE,
                levels=c("0","1"), direction = "<")


roc.list <- list("Strain-Sharing Rate (reciprocated ties)" = logit.base.roc,
                  "Only Socio-Dem Covars" = logit.onlycovars.roc,
                  "SSR + Age + Gender + Household Sharing" = logit.covars.roc,
                  "SSR + All Socio-Dem Covars (reciprocated ties)" = logit.all.roc)
...
## Non-Kin different house Relationships (on reciprocated ties W3) model
```{r plot_roc_non_kin_different_house, echo=FALSE, message=FALSE, warning=FALSE}
roc.plot.nkh.reciprocated <- ggroc(roc.list) +
  theme_minimal() +
  geom_abline(slope=1, intercept = 1, linetype = "dashed", alpha=1, color = "grey") +
  coord_equal()

for(i in 1:4) {
  roc.plot.nkh.reciprocated <- roc.plot.nkh.reciprocated + geom_ribbon(
    data = dat.ci.list[[i]],
    aes(x = x, ymin = lower, ymax = upper),
    fill = i + 1,
    alpha = 0.2,
...
roc.plot.nkh.reciprocated <- roc.plot.nkh.reciprocated + 
theme(plot.title = element_text(size=10, hjust = .5),
      legend.position = c(0.6, 0.2),
      legend.text = element_text(size=10),
      text = element_text(size = 10)) + 
  ...
  ggtitle("Non-Kin Different House Relationships (reciprocated ties)")
...
roc.plot.nkh.reciprocated <- roc.plot.nkh.reciprocated + theme(
  plot.title = element_text(size = 10, hjust = .5),
  legend.title = element_text(size = 8),
  legend.text = element_text(size = 8)
)

svglite('Figures/plot_roc_non_kin_different_house_reciprocated.svg')
roc.plot.nkh.reciprocated
dev.off()

rocs_1 <-ggarrange(roc.plot.reciprocated, roc.plot.nkh.reciprocated, nrow = 2, labels = c("A", "B"), widths = 1)
svglite('Figures/plot_allroc_reciprocated_ties.svg', fix_text_size = FALSE)
rocs_1
dev.off()
```
...
```{r}
metrics_all_predictions_reciprocated_ties <- Downsamp_all_reciprocated_ties |> 
...
metrics_all_predictions_reciprocated_ties
```
```{r}
write_tsv(metrics_all_predictions_reciprocated_ties, 'tables/metrics_all_relationships_network_villages_prediction_reciprocated_ties.tsv')
writexl::write_xlsx(metrics_all_predictions_reciprocated_ties, 'tables/metrics_all_relationships_network_villages_prediction_reciprocated_ties.xlsx')
```

#Predicted network of non-kin relationships
```{r}
metrics_non_kin_prediction_reciprocated_ties <- Downsamp_all_reciprocated_ties_nkh |> 
  inner_join(village_map, by = join_by(village_code_w3==village_code)) |> 
  group_by(village_name_deid) |>  
  summarize( `Percent predicted` = mean(predicted),
             `Percent real` = mean(related),
...
metrics_non_kin_prediction_reciprocated_ties
```

```{r}
write_tsv(metrics_non_kin_prediction_reciprocated_ties, 'tables/metrics_non_kin_network_villages_prediction_reciprocated_ties.tsv')
writexl::write_xlsx(metrics_non_kin_prediction_reciprocated_ties, 'tables/metrics_non_kin_network_villages_prediction_reciprocated_ties.xlsx')
```
