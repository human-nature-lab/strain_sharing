...
foreach(i = c(1:5)) %dopar% {
  ...
  logit.test.1 <- lme4::glmer(related ~ jaccard_sim + (0+jaccard_sim|village_code_w3),
                              data = train1, family = binomial)
  logit.test.2 <- lme4::glmer(related ~ jaccard_sim + (0+jaccard_sim|village_code_w3),
                              data = train2, family = binomial)
  logit.test.3 <- lme4::glmer(related ~ jaccard_sim + (0+jaccard_sim|village_code_w3),
  ...
  logit.test.1.covars <- lme4::glmer(related ~ jaccard_sim + gender.mm + gender.mf +
                        age_difference_abs + average_age + (0+jaccard_sim|village_code_w3),
                              data = train1, family = binomial)
  ...
  logit.test.1.all <- lme4::glmer(related ~ gender.mm + gender.mf + indigenous.both +
                        ...
                        jaccard_sim + (0+jaccard_sim|village_code_w3),
                              data = train1, family = binomial)
...
foreach(i = c(1:5)) %dopar% {
  ...
  logit.test.1 <- lme4::glmer(related ~ bray_curtis_sim + (0+bray_curtis_sim|village_code_w3),
                              data = train1, family = binomial)
  logit.test.2 <- lme4::glmer(related ~ bray_curtis_sim + (0+bray_curtis_sim|village_code_w3),
                              data = train2, family = binomial)
  logit.test.3 <- lme4::glmer(related ~ bray_curtis_sim + (0+bray_curtis_sim|village_code_w3),
                              data = train3, family = binomial)
  preds.1 <- predict(logit.test.1, newdata = test1, type = "response")
  preds.2 <- predict(logit.test.2, newdata = test2, type = "response")
  preds.3 <- predict(logit.test.3, newdata = test3, type = "response")
  ...
  logit.test.1.covars <- lme4::glmer(related ~ bray_curtis_sim + gender.mm + gender.mf +
                        age_difference_abs + average_age + (0+bray_curtis_sim|village_code_w3),
                              data = train1, family = binomial)
  ...
  logit.test.1.all <- lme4::glmer(related ~ gender.mm + gender.mf + indigenous.both +
                        ...
                        same_usage_painkillers + same_usage_antibiotics + same_usage_antidiarrheal + same_usage_antiparasite + same_usage_vitamins + same_usage_zinc + same_usage_antifungal + same_usage_antihypertensives + same_usage_antidiabetics + same_usage_antiacids + same_usage_laxatives +
                        education_difference_abs + average_education + household_same +
                        bray_curtis_sim + (0+bray_curtis_sim|village_code_w3),
                              data = train1, family = binomial)
  logit.test.2.all <- lme4::glmer(related ~ gender.mm + gender.mf + indigenous.both +
                        indigenous.one + religion.same + age_difference_abs +
                        average_age + wealth_difference_abs + average_wealth +
...
svglite("Figures/sfigure8_full.svg", height = 10, width = 8, fix_text_size = FALSE)
ggarrange(rocs_1,
          rocs_2,
          ncol = 1)
dev.off()
