...
meal_pvals_nkh <- meal_pvals_nkh %>% filter(p.signif == "ns") %>%
  mutate(y.position = c(26,28,30,26))
...
meals_all_plot_nkh <- ggplot(SN_Non_Kin_House_Symmetrized, aes(Meals, strain_sharing_rate, color = Meals)) +
  ...
  stat_pvalue_manual(meal_pvals_nkh, label = "p.signif", tip.length = 0, bracket.size = 0.7, size = 5) +
  font("ylab", face = "bold", size = 12) +
    scale_color_manual(values = c("#01b0f8",
                                "#33d600",
                                "#004672",
...
free_time_all_plot_nkh <- ggplot(SN_Non_Kin_House_Symmetrized_2, aes(Free_Time, strain_sharing_rate, color = Free_Time)) +
  ...
  stat_pvalue_manual(free_time_pvals_nkh, label = "p.signif", tip.length = 0, bracket.size = 0.7, size = 5)+
  font("ylab", face = "bold", size = 12) +
    scale_color_manual(values = c("#01b0f8",
                                "#33d600",
                                "#004672",
...
svglite("Figures/sfigure4_full.svg", width = 12, fix_text_size = FALSE)
fig1_nkh
dev.off()
