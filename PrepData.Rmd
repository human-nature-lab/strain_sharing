...
extraction_batch <- bind_rows(read_csv('~/F_BEGHINI_HM/Rn338 gDNA 032822.csv') |> select(`Sample ID`, `WIKI ID`, Plate) |> rename(SampleID = `Sample ID`, WIKI = `WIKI ID`), 
          read_csv('~/F_BEGHINI_HM/Honduras_gDNA_11052020.csv') |> select(SampleID, WIKI, `gDNA Plate`) |> rename(Plate = `gDNA Plate`),
          read_csv('~/F_BEGHINI_HM/Ejl54 stool WGS 010523.csv') |> select(SampleID, WIKI, `gDNA Plate`) |> rename(Plate = `gDNA Plate`)) |> 
...
# #Filter census data to covariates of interest
# hmb_c1 <- hmb_c1 %>%
#   dplyr::select("respondent_master_id", "gender","marital_name", "age_at_survey", "building_id", "building_latitude", "building_longitude","village_name")
# 
# hmb_c2 <- hmb_c2 %>%
#   dplyr::select("respondent_master_id", "gender","marital_name", "age_at_survey", "building_id", "building_latitude", "building_longitude","village_name")
...
hmb_census$village_name[!is.na(hmb_census$village_name) & hmb_census$village_name == "Carrizalito 2"] <- "Carrizalito 1"

#RCT Data
demographics <- read.csv("~/F_BEGHINI_G/WAVE3/v3/honduras_respondents_WAVE3_v3_F_BEGHINI_2021-07-28.csv")
...
New Strain Share
```{r}
test_ssr <- readRDS("data/sharing_rate_with_zero.RDS")
test_ssr_num <- as.matrix(test_ssr)
...
#Remove not get along
all_edges <- all_edges %>% filter(relationship %in% c('child_over12_other_house','closest_friend','father','free_time','mother','partner','personal_private','sibling'))
#Filter to villages for people we have microbiome information from
all_vils <- hmb_census %>%
  filter(respondent_master_id %in% rownames(strain_rate)) %>%
  dplyr::select(village_name) %>% distinct() %>% pull(village_name)

#Calculate coverage rates with network data as denominator
village_names <- c()
for(i in 1:length(all_vils)){
  vil <- all_edges %>% filter(all_edges$village_name_w3 == all_vils[i])
  ...
  print(paste(all_vils[i],":", round(coverage_rate, 3), num_ppl))
  if(!is.na(coverage_rate) & num_ppl>=10){
...
all_edges <- all_edges %>%
  filter(village_name_w3 %in% village_names)
...
```{r}
hmb_census <- hmb_census %>%
  filter(village_name %in% village_names)

#Household data
print("Average Household Size from Microbiome Villages")
hmb_census %>% group_by(building_id) %>%
...
```{r}
#Village size
hmb_census %>% group_by(village_name) %>%
...
covariates <- merge(hmb_census, demographics) %>%
  dplyr::select(-c("building_id_w3", "village_name_w3"))


#Merge on household wealth index data
household_wealth_data <- read.csv("~/F_BEGHINI_G/WAVE3/v3/honduras_households_WAVE3_v3_F_BEGHINI_2021-07-28.csv")
...
covariates <- merge(covariates, household_wealth_data)
...
```{r}
#all_edges %>% filter(village_name_w3 == village_names[6])
...
for(i in 1:length(village_names)){
  ...
  assign(paste0("sn_vil_", i), SN %>% filter(village_name_w3 == village_names[i]))
  
  assign(paste0("sn_vil_all_", i), all_edges %>% filter(village_name_w3 == village_names[i]))
...
hmb_census$village_name_deid <- village_map[hmb_census$village_name]

for(i in 1:nrow(SN)){
  SN$village_name_deid[i] <- village_map[SN$village_name_w3[i]]
}

for(i in 1:nrow(all_edges)){
  all_edges$village_name_deid[i] <- village_map[all_edges$village_name_w3[i]]
}

village_names <- unname(village_map)
