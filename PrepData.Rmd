...
#master_key <- tibble::column_to_rownames(master_key, 'mb_m05')
...
```{r}
# hmb_census |>
#   dplyr::select(mb_m05,village_name_deid, dplyr::contains('mb_b1900')) %>% 
#   pivot_longer(c(-mb_m05,-village_name_deid), names_to='disease', values_drop_na = TRUE) %>% 
#   dplyr::select(-disease) %>% 
#   mutate(value = if_else(value=='None of the above', 'Healthy',value)) %>% 
#   group_by(village_name_deid)  |>
#   count(value) |>
#   pivot_wider(names_from = value, values_from = n) 
#   mutate_at(vars(!matches('mb|vill')), .funs = ~if_else(is.na(.), 0, 1)) %>% 
#   bind_rows(data.frame(mb_m05 = '116-0005-05', Healthy=1))
```
