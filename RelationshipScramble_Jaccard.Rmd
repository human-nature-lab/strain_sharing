---
title: "RelationshipScramble_Jaccard"
...
SN_Rels <- SN_Rels %>% distinct(pair_key, relationship, .keep_all = TRUE) %>% 
  dplyr::select(ego, alter, relationship, jaccard_sim,gender_ego, gender_alter ,real)
```
...
for(i in 1:length(relationships)){
  ...
  for(j in 1:100){
    ...
    for(k in 1:length(scram_alters)){
      ssr_scram[k] <- species_jaccard_sim[rownames(species_jaccard_sim) == scram_egos[k],
                                  colnames(species_jaccard_sim) == scram_alters[k]]
    }
    
    scram_dat <- data.frame(ego = scram_egos,
                            alter = scram_alters,
                            relationship = rep(relationships[i], length(ssr_scram)),
                            jaccard_sim = ssr_scram,
...
add_rels <- data.frame(rbind(sn_vil_rel_1, sn_vil_rel_2, sn_vil_rel_3, sn_vil_rel_4,
  ...
  select(ego, alter, relationship, jaccard_sim, gender_ego, gender_alter)
...
median_ssr <- aggregate(jaccard_sim ~  relationship,
                       SN_Rels %>% filter(real == TRUE),
                       median, na.rm = TRUE)

names(SN_Rels) <- c("ego","alter","relationship",
                    "jaccard_sim","gender_ego","gender_alter","real")



#names(SN_Rels)[names(SN_Rels) == "relationship"] <- "Relationship"
#names(SN_Rels)[names(SN_Rels) == "jaccard_sim"] <- "Strain Sharing Rate"
#names(SN_Rels)[names(SN_Rels) == "real"] <- "Distribution"
#Change based on species
#names(SN_Rels)[names(SN_Rels) == "jaccard_sim"] <- "Bray Curtis Distance"
...
rel_scramb_p_jaccard <- ggboxplot(
  SN_Rels,
  x = "real",
  y = "jaccard_sim",
  color = "real",
